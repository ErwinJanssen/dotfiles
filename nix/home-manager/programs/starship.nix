{
  programs.starship = {
    ...
    settings = {
      ...
      # clear which shell is being used, since Starship is used by all
      # shells.
      shell = {
        disabled = false;
        fish_indicator = "";
        # Default format string, but without an added space on the end. This
        # places the prompt closer to the shell indicator, but only if one
        # is shown. Otherwise most day-to-day usage with Fish shows no shell
